#!/usr/bin/env python3
"""
Cluster RCT Patterns -- SAARC Clinical Trial Equity Analysis
Group: methods-systems | Paper #15
...
Run: python cluster-rct-patterns.py
"""
...
def main():
    ...
    x_years = list(range(2015, 2015 + len(country_values)))
    ...
    # Effect size: high-trial vs low-trial countries
    high = country_values[:len(country_values)//2]
    low = country_values[len(country_values)//2:]
    m1, s1 = sum(high)/len(high), (max(high)-min(high))/2 if len(high)>1 else 1
    m2, s2 = sum(low)/len(low), (max(low)-min(low))/2 if len(low)>1 else 1
    d = cohens_d(m1, max(s1,1), len(high), m2, max(s2,1), len(low))
    print(f"Cohen's d (high vs low trial countries): {d:.3f}")
